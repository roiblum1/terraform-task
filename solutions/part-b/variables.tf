# ==============================================================================
# Part B — every hardcoded value from Part A becomes a variable. The
# password gets special treatment: sensitive = true, and NO default. You
# must never let a real password live in a .tf or .tfvars file.
# ==============================================================================
...
variable "network_id" {
  ...
  description = <<-EOT
    ...
    group to attach the VM to, e.g. "dvportgroup-1001". Given by the
...
variable "datastore_id" {
  type        = string
  description = <<-EOT
    Pre-resolved managed object ID (MoRef) of the datastore to place the VM
...
variable "vm_folder" {
  type        = string
  description = "vSphere VM folder path to place the VM in (pre-created by the instructor)."
}

variable "student_id" {
  type        = string
  description = "Your two-digit student ID, e.g. \"01\". Used to name your VM."
}
